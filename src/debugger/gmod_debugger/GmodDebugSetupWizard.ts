...
function buildSharedAutorunLua(port: number, clientPort: number = DEFAULT_RDB_CLIENT_PORT): string {
    return [
        ...
        '    end',
        '',
        '    local CLIENT_EXEC_PROTOCOL_VERSION = 1',
        '    local MAX_INLINE_CLIENT_EXEC_BYTES = 256 * 1024',
        '    local MAX_FILE_CLIENT_EXEC_BYTES = 1024 * 1024',
        '    local MAX_CLIENT_EXEC_CHUNK_BYTES = 60 * 1024',
        '    local MAX_CLIENT_EXEC_CHUNKS = math.ceil(MAX_FILE_CLIENT_EXEC_BYTES / MAX_CLIENT_EXEC_CHUNK_BYTES)',
        '    local nextClientExecTransferId = 0',
        '',
        '    local function sendClientExec(kind, payload, maxPayloadBytes)',
        '        local totalBytes = #payload',
        '        if totalBytes > maxPayloadBytes then',
        '            return false, "client execution payload exceeds " .. tostring(maxPayloadBytes) .. " bytes"',
        '        end',
        '',
        '        local chunkCount = math.max(1, math.ceil(totalBytes / MAX_CLIENT_EXEC_CHUNK_BYTES))',
        '        if chunkCount > MAX_CLIENT_EXEC_CHUNKS then',
        '            return false, "client execution payload requires too many chunks"',
        '        end',
        '',
        '        nextClientExecTransferId = (nextClientExecTransferId + 1) % 4294967296',
        '        local transferId = nextClientExecTransferId',
        '        for chunkIndex = 0, chunkCount - 1 do',
        '            local startByte = chunkIndex * MAX_CLIENT_EXEC_CHUNK_BYTES + 1',
        '            local chunk = string.sub(payload, startByte, startByte + MAX_CLIENT_EXEC_CHUNK_BYTES - 1)',
        '            local chunkBytes = #chunk',
        '            net.Start("gm_rdb_exec")',
        '            net.WriteUInt(CLIENT_EXEC_PROTOCOL_VERSION, 8)',
        '            net.WriteString(kind)',
        '            net.WriteUInt(transferId, 32)',
        '            net.WriteUInt(totalBytes, 21)',
        ...
        '    end',
        '',
        '    local MAX_RETURN_VALUES = 8',
        '    local MAX_RETURN_TABLE_ENTRIES = 16',
        '    local MAX_RETURN_TABLE_SCAN = 128',
        '    local MAX_RETURN_STRING_BYTES = 1024',
        '',
        ...
        '        end',
        '        if #result > MAX_RETURN_STRING_BYTES then',
        '            return string.sub(result, 1, MAX_RETURN_STRING_BYTES) .. " [TRUNCATED]"',
        '        end',
        '        return result',
        '    end',
        '',
        '    local function summarizeReturnValue(value, depth, seen)',
        ...
        '            return safeToString(value)',
        '        end',
        '        if valueType ~= "table" or depth >= 1 then',
        '            return safeToString(value)',
        '        end',
        '        if seen[value] then',
        '            return "<cycle>"',
        ...
        '            if #entries >= MAX_RETURN_TABLE_SCAN then',
        '                wasTruncated = true',
        '                break',
        '            end',
        '            entries[#entries + 1] = {',
        ...
        '',
        '        local summary = {}',
        '        local retainedEntries = math.min(#entries, MAX_RETURN_TABLE_ENTRIES)',
        '        for entryIndex = 1, retainedEntries do',
        '            local entry = entries[entryIndex]',
        '            summary[entryIndex] = {',
        '                key = entry.key,',
        '                value = summarizeReturnValue(entry.value, depth + 1, seen),',
        '            }',
        '        end',
        '        wasTruncated = wasTruncated or #entries > retainedEntries',
        '        seen[value] = nil',
        '        return { entries = summary, truncated = wasTruncated }',
        '    end',
        '',
        ...
        '',
        '        local callResults = {}',
        '        local callResultCount = 0',
        '        local function collectCallResults(...)',
        '            callResultCount = select("#", ...)',
        '            callResults = {...}',
        '        end',
        '        collectCallResults(xpcall(fn, debug.traceback))',
        '        if not callResults[1] then',
        '            return false, tostring(callResults[2])',
        '        end',
        '',
        '        local returns = {}',
        '        local returnCount = math.max(0, callResultCount - 1)',
        '        local retainedCount = math.min(returnCount, MAX_RETURN_VALUES)',
        '        for returnIndex = 1, retainedCount do',
        '            local value = callResults[returnIndex + 1]',
        '            returns[returnIndex] = {',
        '                index = returnIndex,',
        '                type = type(value),',
        '                value = summarizeReturnValue(value, 0, {}),',
        '            }',
        '        end',
        '        return true, returns, returnCount > retainedCount',
        '    end',
        '',
        ...
        '        end',
        '        if (realm == "client" or realm == "shared") and #code > MAX_INLINE_CLIENT_EXEC_BYTES then',
        '            return false, "client execution payload exceeds " .. tostring(MAX_INLINE_CLIENT_EXEC_BYTES) .. " bytes"',
        '        end',
        '',
        '        local serverExecuted = false',
        '        local clientDispatched = false',
        '        local returns = {}',
        '        local returnsTruncated = false',
        '        if realm == "server" or realm == "shared" then',
        '            local ok, resultOrError, truncated = runServerChunk(code, "gluals_run_lua")',
        '            if not ok then',
        '                return false, resultOrError',
        '            end',
        '            serverExecuted = true',
        '            returns = resultOrError',
        '            returnsTruncated = truncated == true',
        '        end',
        '',
        '        if realm == "client" or realm == "shared" then',
        '            local sent, sendErr = sendClientExec("lua", code, MAX_INLINE_CLIENT_EXEC_BYTES)',
        '            if not sent then',
        ...
        '            clientDispatched = clientDispatched,',
        '            returns = returns,',
        '            returnsTruncated = returnsTruncated,',
        '        })',
        '        if not isstring(encoded) then',
        '            encoded = "{\\"executedAt\\":\\"" .. os.date("!%Y-%m-%dT%H:%M:%SZ") .. "\\""',
        '                .. ",\\"serverExecuted\\":" .. tostring(serverExecuted)',
        '                .. ",\\"clientDispatched\\":" .. tostring(clientDispatched)',
        '                .. ",\\"returns\\":[],\\"returnsTruncated\\":true}"',
        '        end',
        '        return true, encoded',
        '    end',
        '',
        ...
        '        if realm == "client" or realm == "shared" then',
        '            local readErr',
        '            clientCode, readErr = readServerFileForClient(filePath)',
        '            if not isstring(clientCode) then',
        '                return false, tostring(readErr)',
        '            end',
        '            if #clientCode > MAX_FILE_CLIENT_EXEC_BYTES then',
        '                return false, "client execution payload exceeds " .. tostring(MAX_FILE_CLIENT_EXEC_BYTES) .. " bytes"',
        '            end',
        '        end',
        '',
        '        if realm == "server" or realm == "shared" then',
        ...
        '',
        '        if realm == "client" or realm == "shared" then',
        '            local sent, sendErr = sendClientExec("lua", clientCode, MAX_FILE_CLIENT_EXEC_BYTES)',
        '            if not sent then',
        '                return false, sendErr',
        '            end',
        '        end',
        ...
        'if CLIENT then',
        '    local CLIENT_EXEC_PROTOCOL_VERSION = 1',
        '    local MAX_CLIENT_EXEC_PAYLOAD_BYTES = 1024 * 1024',
        '    local MAX_CLIENT_EXEC_CHUNK_BYTES = 60 * 1024',
        '    local MAX_CLIENT_EXEC_CHUNKS = math.ceil(MAX_CLIENT_EXEC_PAYLOAD_BYTES / MAX_CLIENT_EXEC_CHUNK_BYTES)',
        '    local MAX_PENDING_CLIENT_EXEC_TRANSFERS = 4',
        '    local CLIENT_EXEC_TRANSFER_TIMEOUT = 10',
        '    local pendingClientExec = {}',
        '',
        '    local function rejectClientExec(message)',
        '        ErrorNoHalt("[GLuaLS] Client exec rejected: " .. tostring(message) .. "\\n")',
        '    end',
        '',
        '    local function cleanupPendingClientExec(now)',
        '        for transferId, transfer in pairs(pendingClientExec) do',
        '            if now - transfer.lastReceivedAt > CLIENT_EXEC_TRANSFER_TIMEOUT then',
        '                pendingClientExec[transferId] = nil',
        '            end',
        '        end',
        '    end',
        '',
        '    local function pendingClientExecCount()',
        '        local count = 0',
        '        for _ in pairs(pendingClientExec) do',
        '            count = count + 1',
        '        end',
        '        return count',
        '    end',
        '',
        '    timer.Create("gluals_client_exec_cleanup", 1, 0, function()',
        '        cleanupPendingClientExec(RealTime())',
        '    end)',
        '',
        ...
        '        local totalBytes = net.ReadUInt(21)',
        '        local chunkCount = net.ReadUInt(8)',
        '        local chunkIndex = net.ReadUInt(8)',
        '        local chunkBytes = net.ReadUInt(16)',
        '',
        '        if version ~= CLIENT_EXEC_PROTOCOL_VERSION then',
        '            rejectClientExec("unsupported protocol version")',
        '            return',
        '        end',
        '        if kind ~= "lua" then',
        '            rejectClientExec("unknown exec kind: " .. tostring(kind))',
        '            return',
        '        end',
        '        if totalBytes > MAX_CLIENT_EXEC_PAYLOAD_BYTES or chunkCount < 1 or chunkCount > MAX_CLIENT_EXEC_CHUNKS then',
        '            rejectClientExec("invalid transfer bounds")',
        '            return',
        '        end',
        '        if chunkIndex >= chunkCount or chunkBytes > MAX_CLIENT_EXEC_CHUNK_BYTES then',
        '            rejectClientExec("invalid chunk bounds")',
        '            return',
        '        end',
        '',
        '        local expectedChunkCount = math.max(1, math.ceil(totalBytes / MAX_CLIENT_EXEC_CHUNK_BYTES))',
        '        local expectedChunkBytes = totalBytes - chunkIndex * MAX_CLIENT_EXEC_CHUNK_BYTES',
        '        if expectedChunkBytes > MAX_CLIENT_EXEC_CHUNK_BYTES then',
        '            expectedChunkBytes = MAX_CLIENT_EXEC_CHUNK_BYTES',
        '        end',
        '        if chunkCount ~= expectedChunkCount or chunkBytes ~= expectedChunkBytes then',
        '            rejectClientExec("inconsistent chunk metadata")',
        '            return',
        '        end',
        '',
        '        local chunk = net.ReadData(chunkBytes)',
        '        if not isstring(chunk) or #chunk ~= chunkBytes then',
        '            rejectClientExec("truncated chunk data")',
        '            return',
        '        end',
        '',
        '        local now = RealTime()',
        '        cleanupPendingClientExec(now)',
        '        local transfer = pendingClientExec[transferId]',
        '        if transfer and (transfer.kind ~= kind or transfer.totalBytes ~= totalBytes or transfer.chunkCount ~= chunkCount) then',
        '            pendingClientExec[transferId] = nil',
        '            rejectClientExec("conflicting transfer metadata")',
        '            return',
        '        end',
        '        if not transfer then',
        '            if pendingClientExecCount() >= MAX_PENDING_CLIENT_EXEC_TRANSFERS then',
        '                rejectClientExec("too many pending transfers")',
        '                return',
        '            end',
        '            transfer = {',
        '                kind = kind,',
        ...
        '                lastReceivedAt = now,',
        '            }',
        '            pendingClientExec[transferId] = transfer',
        '        end',
        '',
        '        local chunkSlot = chunkIndex + 1',
        '        local existingChunk = transfer.chunks[chunkSlot]',
        '        if existingChunk then',
        '            if existingChunk ~= chunk then',
        '                pendingClientExec[transferId] = nil',
        '                rejectClientExec("conflicting duplicate chunk")',
        '                return',
        '            end',
        '            transfer.lastReceivedAt = now',
        '            return',
        '        end',
        '',
        '        transfer.chunks[chunkSlot] = chunk',
        '        transfer.receivedBytes = transfer.receivedBytes + chunkBytes',
        '        transfer.receivedCount = transfer.receivedCount + 1',
        '        transfer.lastReceivedAt = now',
        '        if transfer.receivedBytes > transfer.totalBytes then',
        '            pendingClientExec[transferId] = nil',
        '            rejectClientExec("transfer exceeds declared size")',
        '            return',
        '        end',
        ...
        '        if transfer.receivedBytes ~= transfer.totalBytes then',
        '            pendingClientExec[transferId] = nil',
        '            rejectClientExec("transfer size mismatch")',
        '            return',
        '        end',
        '',
        '        local payload = table.concat(transfer.chunks)',
        '        pendingClientExec[transferId] = nil',
        '        if #payload ~= totalBytes then',
        '            rejectClientExec("reassembled payload size mismatch")',
        '            return',
        '        end',
        '',
